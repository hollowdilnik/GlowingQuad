...
To make a custom mesh, define the shape using the spline in an instance of `/ShaderGlow/BP_QuadMaker`, select its procedural mesh and click on `Create StaticMesh`.
...
- Keep the spline points in the same horizontal plane. If you want to create a spline mesh on a slanted / vertical surface, align the QuadMaker with the surface first.
